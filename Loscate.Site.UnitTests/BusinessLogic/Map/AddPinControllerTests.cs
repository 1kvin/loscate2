...
{
    [TestClass()]
    public class AddPinControllerTests
    {
        ...
        [TestMethod]
        public void PostOutOfLimitTest()
        {
            ...
            var pins = new List<Pin>() { pin, pin, pin }.AsQueryable();
            ...
            var controller = new AddPinController(mockContextDb.Object, mockUserService.Object); ;
            var result = controller.Post(testPinFullName, testPinShortName, testPinLatitude.ToString(), testPinLongitude.ToString(), testPinPhoto);

            Assert.IsNotNull(result);
            Assert.AreEqual(result, "LIMIT");
        }
        [TestMethod()]
        public void PostCorrectPinTest()
        {
            var testUser = new FirebaseUser { Id = testUserId, Uid = testUserUid };
            ...
            var pins = new List<Pin>(){}.AsQueryable();
            var pinsMockSet = new Mock<DbSet<Pin>>();
            pinsMockSet.As<IQueryable<Pin>>().Setup(m => m.Provider).Returns(pins.Provider);
            pinsMockSet.As<IQueryable<Pin>>().Setup(m => m.Expression).Returns(pins.Expression);
            pinsMockSet.As<IQueryable<Pin>>().Setup(m => m.ElementType).Returns(pins.ElementType);
