...
using Loscate.Site.Controllers.Social.Dialog;
using System;
...
namespace Loscate.Site.Controllers.Social.Dialog.Tests
{
    [TestClass()]
    public class GetUserDialogsControllerTests
    {
        ...
        [TestMethod()]
        public void GetEmptyDialogTest()
        {
            ...
            var controller = new GetUserDialogsController(mockContextDb.Object, mockUserService.Object);
            var result = controller.Get();
            Assert.IsNotNull(result);
            Assert.AreEqual(result[0].LastMessage.SendUser.Name, testUser1.Name);
            Assert.AreEqual(result[0].LastMessage.Text, "Нет сообщений");
        }


        [TestMethod()]
        public void GetCorrectDialogTest()
        {
            ...
            var result = controller.Get();
            Assert.IsNotNull(result);
            Assert.AreEqual(result[0].Companion.Name, testUser2.Name);
            Assert.AreEqual(result[0].LastMessage.Text, testMessageText);
        }
    }
}