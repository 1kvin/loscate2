    ...
    public class AddPinViewModel : BaseViewModel
    {
        ...
        private async Task AddPin()
        {
            ...
            else
            {
                ...
                if (res == "LIMIT")
                {
                    await Application.Current.MainPage.DisplayAlert("Предупреждение", "Достигнуто максимальное колличество добавленных точек за день. Повторите завтра.", "ОК");
                }
                else if (res != "OK")
                {
